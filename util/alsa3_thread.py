...
class MixerThread:
    ...
    def set_mute(self, mute: bool) -> None:
        if self.mixer:
            self.mixer.setmute(int(mute))
            logger.info("Set mute to %s", mute)
        else:
    ...
    def handle_message_event(self, read_fd) -> None:
        ...
        if data:
            ...
            elif message.get("command") == "set_mute":
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            if count % 5 == 0:
                volume = 50
                message = {"command": "set_volume", "volume": volume}
                os.write(write_fd, json.dumps(message).encode('utf-8'))
            else:
                muted = not muted
                message = {"command": "set_mute", "mute": muted}
                os.write(write_fd, json.dumps(message).encode('utf-8'))
